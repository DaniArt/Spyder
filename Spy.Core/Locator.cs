...
public static class LocatorGenerator
{
    ...
    public static Locator Build(SpyElement element)
    {
        ...
        bool preferVclFallback =
            (element.VclChain?.Count ?? 0) > 0 ||
            IsVclClass(element.Win32Class) ||
            IsVclClass(element.UiaClassName) ||
            (!string.IsNullOrWhiteSpace(element.UiaFrameworkId) &&
             element.UiaFrameworkId.Contains("vcl", StringComparison.OrdinalIgnoreCase));
        ...
        foreach (var node in chain)
        {
            ...
            var isVclNode = IsVclClass(cls) || preferVclFallback;
        ...
        // 3) сам элемент
        var isVclLeaf = IsVclClass(element.UiaClassName ?? element.Win32Class) || preferVclFallback;
        ...
        if (!string.IsNullOrWhiteSpace(leafName))
        {
            ...
            var leafType = IsVclClass(leafClass) || preferVclFallback
                ? LocatorStepType.VCLObject
                : LocatorStepType.Client;
